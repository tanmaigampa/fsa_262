...
def display_ratios(canonical_model, ratios):
    """Display financial ratios with expandable formula details"""
    ...
    ratio_formulas = {
        ...
        'Cash Ratio': {
            ...
            'inputs': ['cash_ending', 'current_liabilities']
        },
        'Working Capital Ratio': {
            'formula': 'Working Capital / Total Assets',
            'inputs': ['working_capital', 'total_assets']
        },
        'Debt to Equity': {
    ...
    for category, ratio_list in categories.items():
        ...
        if category_data:
            ...
            # Add expandable section for formula breakdown
            for ratio_name in ratio_list:
                if ratio_name in ratios and ratio_name in ratio_formulas:
                    with st.expander(f"📐 {ratio_name} - Formula & Inputs"):
                        formula_info = ratio_formulas[ratio_name]
                        
                        # Show formula
                        st.markdown(f"**Formula:** `{formula_info['formula']}`")
                        
                        # Show inputs for each year
                        st.markdown("**Input Values by Year:**")
                        
                        input_data = {}
                        for input_field in formula_info['inputs']:
                            # Get clean field name for display
                            field_display = input_field.replace('_', ' ').title()
                            input_values = []
                            
                            for year in years:
                                value = canonical_model.get_value(input_field, year)
                                input_values.append(value)
                            
                            input_data[field_display] = input_values
                        
                        # Create input DataFrame
                        input_df = pd.DataFrame(input_data, index=years).T
                        
                        st.dataframe(
                            input_df.style.format("{:,.2f}", na_rep="N/A"),
                            use_container_width=True
                        )
                        
                        # Show calculation example for most recent year
                        if years:
                            latest_year = years[-1]
                            st.markdown(f"**Example Calculation ({latest_year}):**")
                            ...
                            for input_field in formula_info['inputs']:
                                value = canonical_model.get_value(input_field, latest_year)
                                field_display = input_field.replace('_', ' ').title()
                                
                                if value is not None:
                                    calc_parts.append(f"{field_display} = {value:,.2f}")
                                else:
                                    calc_parts.append(f"{field_display} = N/A")
                            ...
                            # Show result
                            result = ratios[ratio_name].get(latest_year)
                            if result is not None and all_values_present:
                                st.markdown(f"**Result:** `{result:.2f}`")
                            else:
                                st.markdown(f"**Result:** `N/A` (missing input data)")
        else:
